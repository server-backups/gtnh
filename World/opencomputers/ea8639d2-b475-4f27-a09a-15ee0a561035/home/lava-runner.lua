...
local map_meta={
  ...
  lava_update=function(self)
    ...
    if self:get(self.player.x,self.player.y).type==-3 then gpu.setResolution(gpu.maxResolution()) term.clear() print("You've been boiled alive in lava! You've reached the "..self.player.level.." level this time.")  os.exit() end
  end,
  update=function(self)
    ...
    if code==16 then gpu.setResolution(gpu.maxResolution()) term.clear() print("You've quit the game on the "..self.player.level.." level.")  os.exit() else if keys[code] then keys[code]() end end
  end,
